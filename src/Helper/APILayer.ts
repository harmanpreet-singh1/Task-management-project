...
export class BoardAPI {
  async fetchBoardList(): Promise<IList[]> {
    const apiData: IList[] = ApiMockResponse;
    let BoardList: IList[] = [];
    ...
    if (localStorage.getItem(LocalStorageKeyName)) {
      const localStorageData: IList[] = JSON.parse(
...
//Business Layer
export async function fetchBoardList(): Promise<IList[]> {
  const api = new BoardAPI();
  return api.fetchBoardList();
}
export function updateLocalStorageBoards(boards: IList[]) {
